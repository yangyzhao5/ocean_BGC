...
!       ndet_fast: fast-sinking nitrogen detritus ! YZ: fast-sinking, 07/07/2025
...
!       pdet_fast: fast-sinking phosphorous detritus ! YZ: fast-sinking, 07/07/2025
...
!       implement the N2O formulation following Suntharalingam et al. (2000) based on oxygen consumption below the
!       euphotic zone, refer to diagnostic N2O as n2o_diag. The N2O production with a constant background yield via
!       nitrification is referred to the nitrification pathway, i.e., n2o_diag_nit, and N2O production with an oxygen-
!       dependant yield is referred to the low-oxygen pathway, i.e., n2o_diag_lowo2, following Jin and Gruber (2003).
!       Following Suntharalingam et al. (2012, 2019), N2O loss in anoxic conditions is represented as (−1) x the value 
!       of total N2O production below oxygen threshold levels of o2_max_diag; i.e., all N2O produced in these zones is 
!       assumed to be consumed by denitrification processes, and this represents an upper bound on N2O loss in these zones.
!
!       three newly added tracers:
!       n2o_diag: diagnostic nitrous oxide produced based on oxygen consumption
!       n2o_diag_nit: diagnostic nitrous oxide produced via nitrification
!       n2o_diag_lowo2: diagnostic nitrous oxide produced via low-oxygen pathway
! } YZ
!
...
!
! YZ: N2O_module, 13/06/2025 {
!  <DATA NAME="do_n2o" TYPE="logical">
!  If true, then simulate nitrous oxide. Includes 1 prognostic tracers, N2O.
!  </DATA>
!
!  < DATA NAME="do_n2o_decomp" TYPE="logical">
!  If true, then conduct nitrous oxide tracer decomposition. Incudes 4
!  prognostic tracers, N2O_nit, N2O_denit, N2O_atm, N2O_sed
!  </DATA>
! } YZ
!
!</NAMELIST>
...
! <REFERENCE>
!
! Zhao et al. (2025) https://doi.org/10.1029/2024GB008481
!
...
module generic_COBALT
  ...
  use constants_mod,     only: WTMCO2, WTMO2,WTMN,rdgas,wtmair,WTMN2O   ! YZ: N2O_module, 13/06/2025
  ...
  namelist /generic_COBALT_nml/ co2_calc, do_14c, do_nh3_atm_ocean_exchange, scheme_nitrif, debug, &
     ...
     do_n2o,do_n2o_decomp,do_n2o_diag  ! YZ: N2O_module, 13/06/2025 ! YZ: N2O_diag, 06/12/2025
...
contains
  ...
  subroutine generic_COBALT_register(tracer_list)
    ...
    ! YZ: N2O_module, 13/06/2025 {
    if (do_n2o) then
      write (stdoutunit,*) trim(note_header), 'Simulating nitrous oxide'
      if (do_n2o_decomp) then
        write (stdoutunit,*) trim(note_header), 'Conducting nitrous oxide decomposition'
      endif
      ! YZ: N2O_diag, 06/12/2025
      if (do_n2o_diag) then
        write (stdoutunit,*) trim(note_header), 'Conducting nitrous oxide diagnostics'
      endif ! } YZ
    endif ! } YZ
...
# include "version_variable.h"
  ...
  subroutine user_add_params(param_file)
    ...
    call get_param(param_file, "generic_COBALT", "wsink_fast",  cobalt%wsink_fast, "wsink_fast", units="m day-1", &
                   default= 1000.0, scale = I_sperd ) ! s-1 ! } YZ
    ...
    ! YZ: N2O_module, 13/06/2025 {
    !------------------------------------------------------------------------------------------------------------------
    ! Coefficients for N2O saturation based on Weiss and Price (1980). Nitrous oxide solubility in water and seawater.
    ! Marine chemistry, 8(4), 347-359.
    ! https://www.sciencedirect.com/science/article/pii/0304420380900249
    ! Coefficients of K0 in mol/l/atm in Table II were used.
    !------------------------------------------------------------------------------------------------------------------
    call get_param(param_file, "generic_COBALT", "a_1_n2o", cobalt%a_1_n2o, "N2O sat. regression coefficient", &
                   units="mol L-1 (atm)-1", default= -62.7062)
    call get_param(param_file, "generic_COBALT", "a_2_n2o", cobalt%a_2_n2o, "N2O sat. regression coefficient", &
                   units="mol L-1 (atm)-1", default= 97.3066)
    call get_param(param_file, "generic_COBALT", "a_3_n2o", cobalt%a_3_n2o, "N2O sat. regression coefficient", &
                   units="mol L-1 (atm)-1", default= 24.1406)
    call get_param(param_file, "generic_COBALT", "b_1_n2o", cobalt%b_1_n2o, "N2O sat. regression coefficient", &
                   units="mol L-1 (atm salt)-1", default= -5.8420e-02)
    call get_param(param_file, "generic_COBALT", "b_2_n2o", cobalt%b_2_n2o, "N2O sat. regression coefficient", &
                   units="mol L-1 (atm salt)-1", default=  3.3193e-02)
    call get_param(param_file, "generic_COBALT", "b_3_n2o", cobalt%b_3_n2o, "N2O sat. regression coefficient", &
                   units="mol L-1 (atm salt)-1", default= -5.1313e-03)    ! } YZ
    !------------------------------------------------------------------------------------------------------------------
    ...
    if (trim(as_param_cobalt) == "W92") then
        ...
        call get_param(param_file, "generic_COBALT", "a5_o2", cobalt%a5_o2, "O2 Schmidt # regression coefficient", &
                       units="deg. C-4", default= 0.0)       ! Not used for W92
        ! YZ: N2O_module, 13/06/2025 {
        !  Compute the Schmidt number of N2O in seawater using the W92_extended relationships as above
        call get_param(param_file, "generic_COBALT", "a1_n2o", cobalt%a1_n2o, "N2O Schmidt # regression coefficient", &
                       units="dimensionless", default= 2301.1)
        call get_param(param_file, "generic_COBALT", "a2_n2o", cobalt%a2_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-1", default= -151.1)
        call get_param(param_file, "generic_COBALT", "a3_n2o", cobalt%a3_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-2", default= 4.7364)
        call get_param(param_file, "generic_COBALT", "a4_n2o", cobalt%a4_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-3", default= -5.9431e-02)
        call get_param(param_file, "generic_COBALT", "a5_n2o", cobalt%a5_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-4", default= 0.0)       ! Not used for W92  ! } YZ
        ...
        ! YZ: N2O_module, 13/06/2025 {
        !  Compute the Schmidt number of N2O in seawater using the W14 relationships as above
        call get_param(param_file, "generic_COBALT", "a1_n2o", cobalt%a1_n2o, "N2O Schmidt # regression coefficient", &
                       units="dimensionless", default= 2356.2)
        call get_param(param_file, "generic_COBALT", "a2_n2o", cobalt%a2_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-1", default= -166.38)
        call get_param(param_file, "generic_COBALT", "a3_n2o", cobalt%a3_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-2", default= 6.3952)
        call get_param(param_file, "generic_COBALT", "a4_n2o", cobalt%a4_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-3", default= -0.13422)
        call get_param(param_file, "generic_COBALT", "a5_n2o", cobalt%a5_n2o, "N2O Schmidt # regression coefficient", &
                       units="deg. C-4", default= 0.0011506) ! } YZ
    else
    ...
    ! YZ: N2O_module, 13/06/2025 {
    !   Denitrification is explicitly divided into two steps: nitrate reduction to N2O and N2O
    !   reduction to N2. Nitrate reduction to nitrite is implicitly included and the nitrite that
    !   does not go into N2O by heterotrophic denitrification or N2 by anammox is re-oxidized
    !   back to nitrate.
    !   Nitrate reduction to N2O (denit1):
    !   C106H172O38N16 + 118*NO3- + 134*H+ <-> 106*CO2 + 16*NH4+ + 59*N2O + 121*H2O   
    !   Effect is to increase alkalinity by 134/59 = 2.271 N2O equivalents.
    !
    !   N2O reduction to dinitrogen (denit2):
    !   C106H172O38N16 + 236*N2O + 16*H+ <-> 106*CO2 + 16*NH4+ + 236*N2 + 62*H2O
    !   Effect is to increase alkalinity by 16/236 = 0.068 N2O equivalents.
    !
    !   Nitrate reduction to nitrite (denit0):
    !   C106H172O38N16 + 236*NO3- + 16*H+ <-> 106*CO2 + 16*NH4+ + 236*NO2- + 62*H2O
    !   Nitrite re-oxidation to nitrate:
    !   2*NO2- + O2 --> 2*NO3-
    !   Total effect is to increase alkalinity by 16 NH4+ equivalents.   } YZ
    !
    ! Anammox:
    ...
    ! YZ: N2O_module, 13/06/2025 {
    call get_param(param_file, "generic_COBALT", "alk_2_n2o_denit1", cobalt%alk_2_n2o_denit1, &
                   "moles alkalinity created per mole N2O produced during nitrate reduction to N2O", &
                   units="mol alk mol N2O-1 ", default= 134.0/59.0)
    call get_param(param_file, "generic_COBALT", "alk_2_n2o_denit2", cobalt%alk_2_n2o_denit2, &
                   "moles alkalinity created per mole N2O consumed during N2O reduction to N2", &
                   units="mol alk mol N2O-1 ", default= 16.0/236.0)  ! } YZ
    !
    !-----------------------------------------------------------------------
    ...
    call get_param(param_file, "generic_COBALT", "frac_fastsinking", cobalt%frac_fastsinking, &
                   "Fraction of N and P detritus higher predators that is fast-sinking", units="none", default=1.0) ! } YZ
    ...
    call get_param(param_file, "generic_COBALT", "o2_min", cobalt%o2_min, &
                   "Minimum O2 for aerobic remineralization", units="mol O2 kg-1", default= 1.0e-6) ! YZ: 06/12/2025, change from 0.8e-6
    ...
    ! YZ: N2O_module, 13/06/2025 {
    call get_param(param_file, "generic_COBALT", "gamma_ndet_denit1", cobalt%gamma_ndet_denit1, &
                   "Remineralization rate for unprotected organic matter during nitrate reduction to N2O", &
                   units="s-1", default=cobalt%wsink/2500.0)
    call get_param(param_file, "generic_COBALT", "gamma_ndet_denit2", cobalt%gamma_ndet_denit2, &
                   "Remineralization rate for unprotected organic matter during N2O reduction to N2", & 
                   units="s-1", default=cobalt%wsink/625.0)  ! } YZ
    ...
    call get_param(param_file, "generic_COBALT", "k_o2_amx", cobalt%k_o2_amx, &
                   "oxygen half-saturation for anammox", units="mol O2 kg-1", default= 6.0e-6)  ! } YZ
    ...
    !-----------------------------------------------------------------------
    !
    call get_param(param_file, "generic_COBALT", "alpha_n2o",  cobalt%alpha_n2o, &
                   "scalar multiplier for N2O prod., O2 dependency", units="mol N2O (mol N)-1 * (mol O2 kg-1)", default= 2.0e-09)
    call get_param(param_file, "generic_COBALT", "beta_n2o",  cobalt%beta_n2o, &
                   "scalar multiplier for N2O production", units="mol N2O (mol N)-1", default= 8.0e-04)
    call get_param(param_file, "generic_COBALT", "o2_max_denit",  cobalt%o2_max_denit, &
                   "upper oxygen threshold for denitrification", units="mol O2 kg-1", default= 20.0e-06)
    call get_param(param_file, "generic_COBALT", "k_o2_denit1",  cobalt%k_o2_denit1, &
                   "oxygen poisoning for during nitrate reduction to N2O", units="mol O2 kg-1", default= 2.3e-06)
    call get_param(param_file, "generic_COBALT", "k_o2_denit2",  cobalt%k_o2_denit2, &
                   "oxygen poisoning for during N2O reduction to N2", units="mol O2 kg-1", default= 0.68e-06)
    call get_param(param_file, "generic_COBALT", "k_n2o_denit",  cobalt%k_n2o_denit, &
                   "half-saturation coefficient of N2O during N2O reduction to N2", units="mol N2O kg-1", default= 1.90e-07)
    call get_param(param_file, "generic_COBALT", "n2o_2_n_sed",  cobalt%n2o_2_n_sed, &
                   "scalar multiplier for N2O production from sedimentary denitrification", &
                   units="mol N2O (mol N)-1", default= 0.5*3.0e-03)  ! } YZ
    ! YZ: N2O_diag, 06/12/2025 {
    call get_param(param_file, "generic_COBALT", "alpha_n2o_diag",  cobalt%alpha_n2o_diag, &
                   "scalar multiplier for diagnostic N2O prod., background", units="mol N2O (mol O2 consumed)-1", default= 0.75e-04)
    call get_param(param_file, "generic_COBALT", "beta_n2o_diag",  cobalt%beta_n2o_diag, &
                   "scalar multiplier for diagnostic N2O prod., O2 dependency", units="mol N2O (mol O2 consumed)-1", default= 1.0e-02)
    call get_param(param_file, "generic_COBALT", "lamda_n2o_diag",  cobalt%lamda_n2o_diag, &
                   "scalar multiplier for diagnostic N2O prod.", units="dimensionless", default= 0.1)
    call get_param(param_file, "generic_COBALT", "k_n2o_diag",  cobalt%k_n2o_diag, &
                   "first-order rate constant of N2O consumption", units="s-1", default= 0.138, scale = I_spery)
    call get_param(param_file, "generic_COBALT", "o2_offset_diag", cobalt%o2_offset_diag, &
                   "O2 threshold for reduced net diagnostic N2O production", units="mol O2 kg-1", default= 1.0e-6) 
    call get_param(param_file, "generic_COBALT", "o2_max_diag", cobalt%o2_max_diag, &
                   "Maximum O2 for diagnostic N2O consumption", units="mol O2 kg-1", default= 5.0e-6) ! } YZ
    !
    !-----------------------------------------------------------------------
    ! Miscellaneous
    !-----------------------------------------------------------------------
...
!            implementation='duce',   &
    ...
    if (do_n2o) then !{
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_nit',                                         &
        longname   = 'Nitrous oxide from nitrification',                &
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_nit_flux',                                &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_denit',                                       &
        longname   = 'Nitrous oxide from denitrification',              &
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_denit_flux',                              &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_atm',                                         &
        longname   = 'Nitrous oxide from the atmosphere',               &
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_atm_flux',                                &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_sed',                                         &
        longname   = 'Nitrous oxide from sediments',                    &
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_sed_flux',                                &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_diag',                                        &
        longname   = 'Diagnostic nitrous oxide',                        &
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_diag_flux',                               &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_diag_nit',                                    &
        longname   = 'Diagnostic nitrous oxide from nitrification',     &
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_diag_nit_flux',                           &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
      ...
      call g_tracer_add(tracer_list,package_name,                       &
        name       = 'n2o_diag_lowo2',                                  &
        longname   = 'Diagnostic nitrous oxide from low-oxygen pathway',&
        units      = 'mol/kg',                                          &
        prog       = .true.,                                            &
        flux_gas   = .true.,                                            &
        flux_gas_name  = 'n2o_diag_lowo2_flux',                         &
        flux_gas_type  = 'air_sea_gas_flux_generic',                    &
        flux_gas_molwt = WTMN2O,                                        &
        flux_gas_param = (/ as_coeff_cobalt, 9.7561e-06 /),             &
        flux_gas_restart_file  = 'ocean_cobalt_airsea_flux.res.nc',     &
  ...
  subroutine generic_COBALT_update_from_bottom(tracer_list, dt, tau, model_time,Temp, Salt, rho_dzt, dzt, ilb, jlb)
    ...
    model_time, rmask = grid_tmask(:,:,1),&
    is_in=isc, js_in=jsc,ie_in=iec, je_in=jec) ! } YZ
    ...
    call g_tracer_get_values(tracer_list,'ndet_fast_btf','field',cobalt%f_ndet_fast_btf,isd,jsd) ! YZ: fast-sinking, 07/07/2025
    call g_tracer_get_values(tracer_list,'pdet_btf','field',cobalt%f_pdet_btf,isd,jsd)
    call g_tracer_get_values(tracer_list,'pdet_fast_btf','field',cobalt%f_pdet_fast_btf,isd,jsd) ! YZ: fast-sinking, 07/07/2025
    ...
    do j = jsc, jec; do i = isc, iec  !{
       if (grid_kmt(i,j) .gt. 0) then !{
          cobalt%fntot_btm(i,j) = cobalt%f_ndet_btf(i,j,1) + cobalt%f_ndet_fast_btf(i,j,1) + cobalt%f_ndi_btf(i,j,1) + & ! YZ: fast-sinking
            cobalt%f_nsm_btf(i,j,1) + cobalt%f_nmd_btf(i,j,1) + cobalt%f_nlg_btf(i,j,1)
          cobalt%fptot_btm(i,j) = cobalt%f_pdet_btf(i,j,1) + cobalt%f_pdet_fast_btf(i,j,1) + cobalt%f_pdi_btf(i,j,1) + & ! YZ: fast-sinking
  ...
  !     ilb,jlb,tau,dt,grid_dat,model_time,nbands,max_wavelength_band,sw_pen_band,opacity_band,internal_heat,frunoff)
    ...
    real :: log100,tkn,tkn100,tkn1002,logtkn  ! YZ: N2O_module, 13/06/2025
    ...
    real :: fo2_dependence  ! YZ: N2O_diag, 06/12/2025
   ...
   call g_tracer_get_values(tracer_list,'di14c' ,'field', cobalt%f_di14c,isd,jsd,positive=.true.)
    ...
    call g_tracer_get_values(tracer_list,'ndet_fast','field',cobalt%f_ndet_fast ,isd,jsd,positive=.true.) ! YZ: fast-sinking, 07/07/2025
    ...
    call g_tracer_get_values(tracer_list,'pdet_fast','field',cobalt%f_pdet_fast ,isd,jsd,positive=.true.) ! YZ: fast-sinking, 07/07/2025
    ...
    if (do_n2o) then !{
      ...
      call g_tracer_get_values(tracer_list,'n2o_atm'  ,'field',cobalt%f_n2o_atm  ,isd,jsd,positive=.true.)
      call g_tracer_get_values(tracer_list,'n2o_sed'  ,'field',cobalt%f_n2o_sed  ,isd,jsd,positive=.true.)
      endif !}
      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      call g_tracer_get_values(tracer_list,'n2o_diag' ,'field',cobalt%f_n2o_diag ,isd,jsd,positive=.true.)
      call g_tracer_get_values(tracer_list,'n2o_diag_nit' ,'field',cobalt%f_n2o_diag_nit ,isd,jsd,positive=.true.)
      call g_tracer_get_values(tracer_list,'n2o_diag_lowo2' ,'field',cobalt%f_n2o_diag_lowo2 ,isd,jsd,positive=.true.)
      endif !} ! } YZ
    endif !} ! } YZ
    !
    ...
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec
       ...
       cobalt%jprod_ndet_fast(i,j,k) = 0.0 ! YZ: fast-sinking, 07/07/2025
       cobalt%jprod_pdet(i,j,k) = 0.0
       cobalt%jprod_pdet_fast(i,j,k) = 0.0 ! YZ: fast-sinking, 07/07/2025
       ...
       ! YZ: N2O_module, 13/06/2025 {
       if (do_n2o) then !{
         cobalt%jremin_fedet(i,j,k) = 0.0
         cobalt%jremin_ndet_denit1(i,j,k) = 0.0
         cobalt%jremin_ndet_denit2(i,j,k) = 0.0
         cobalt%jremin_ndet_denit0(i,j,k) = 0.0
         ...
         cobalt%jsink_n2o_atm(i,j,k) = 0.0
         cobalt%jsink_n2o_sed(i,j,k) = 0.0
         endif !}
         ! YZ: N2O_diag, 06/12/2025 {
         if (do_n2o_diag) then !{
         cobalt%jprod_n2o_diag(i,j,k) = 0.0
         cobalt%jprod_n2o_diag_nit(i,j,k) = 0.0
         cobalt%jprod_n2o_diag_lowo2(i,j,k) = 0.0
         cobalt%jsink_n2o_diag(i,j,k) = 0.0
         cobalt%jsink_n2o_diag_nit(i,j,k) = 0.0
         cobalt%jsink_n2o_diag_lowo2(i,j,k) = 0.0
         endif !} ! } YZ
       endif !} ! } YZ
    enddo;  enddo ;  enddo !} i,j,k
!
...
!
    ...
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec   !{
       ...
       bact(1)%juptake_ldon(i,j,k) = vmax_bact*bact(1)%temp_lim(i,j,k)*bact(1)%ldonlim(i,j,k)* &
               bact(1)%o2lim(i,j,k)*bact(1)%no3lim(i,j,k)*bact(1)%f_n(i,j,k)  ! } YZ
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec; !{
       ...
       prey_vec(9) = max(cobalt%f_ndet(i,j,k) + cobalt%f_ndet_fast(i,j,k) - cobalt%refuge_conc,0.0) ! YZ: fast-sinking, 07/07/2025
       ...
       prey_p2n_vec(9)  = (cobalt%f_pdet(i,j,k) + cobalt%f_pdet_fast(i,j,k))/ &
                          (cobalt%f_ndet(i,j,k) + cobalt%f_ndet_fast(i,j,k) + epsln) ! YZ: fast-sinking, 07/07/2025
       ...
       prey_fe2n_vec(9) = cobalt%f_fedet(i,j,k)/(cobalt%f_ndet(i,j,k) + cobalt%f_ndet_fast(i,j,k) + epsln) ! YZ: fast-sinking, 07/07/2025
       prey_si2n_vec(2) = phyto(LARGE)%q_si_2_n(i,j,k)
       prey_si2n_vec(3) = phyto(MEDIUM)%q_si_2_n(i,j,k)
       prey_si2n_vec(9) = cobalt%f_sidet(i,j,k)/(cobalt%f_ndet(i,j,k) + cobalt%f_ndet_fast(i,j,k) + epsln) ! YZ: fast-sinking, 07/07/2025
...
!------------------------------------------------------------------------------------
    ...
    !
    ! YZ: N2O_module, 13/06/2025 {
    ! Aerobic and anaerobic remineralization is allowed to co-exist when oxygen concentrations are between 10 nmol/kg
    ! and about 10-20 umol/kg. Similar to aerobic remineralization, anaerobic remineralization is also dependent on
    ! temperature. However, differing from aerobic remineralization, the oxygen inhibitation on anaerobic
    ! remineralization is exponentially dependent on oxygen concentrations (Bianchi et al., 2023).
    ! Anaerobic remineralization (i.e., heterotrophic denitrification) is divided into two steps in case of simulating
    ! N2O cycles: 1) denit1 - nitrate reduction to N2O and 2) denit2 - N2O reduction to N2.
    ! Note that nitrate reduction to nitrite (denit0) is implicitly represented, allowing nitrite that does not go into
    ! N2O via denit1 or N2 via anammox is re-oxidized back to nitrate.
    ! Note the ndet consumption by nitrate reduction for anammox should be subducted here.
    !
    ! References:
    ! Bianchi et al., 2023: https://doi.org/10.5194/gmd-16-3581-2023
    !
    if (do_n2o) then !{
    do k=1,nk ; do j=jsc,jec ; do i=isc,iec  !{
       cobalt%expkreminT(i,j,k) = exp(cobalt%kappa_remin * Temp(i,j,k))
       ...
       if (cobalt%f_o2(i,j,k) .lt. cobalt%o2_max_denit) then  !{
          ! Nitrate reduction to N2O
          cobalt%jremin_ndet_denit1(i,j,k) = cobalt%gamma_ndet_denit1 * cobalt%expkreminT(i,j,k) * &
               exp( - cobalt%f_o2(i,j,k) / cobalt%k_o2_denit1 ) * &
               cobalt%f_no3(i,j,k) / (cobalt%k_no3_denit + cobalt%f_no3(i,j,k))* &
               max(0.0, cobalt%f_ndet(i,j,k) - &
               cobalt%rpcaco3*(cobalt%f_cadet_arag(i,j,k) + cobalt%f_cadet_calc(i,j,k)) - &
               cobalt%rplith*cobalt%f_lithdet(i,j,k) - cobalt%rpsio2*cobalt%f_sidet(i,j,k) )
          ! N2O reduction to N2
          cobalt%jremin_ndet_denit2(i,j,k) = cobalt%gamma_ndet_denit2 * cobalt%expkreminT(i,j,k) * &
               exp( - cobalt%f_o2(i,j,k) / cobalt%k_o2_denit2 ) * &
               cobalt%f_n2o(i,j,k) / (cobalt%k_n2o_denit + cobalt%f_n2o(i,j,k))* &
               max(0.0, cobalt%f_ndet(i,j,k) - &
               cobalt%rpcaco3*(cobalt%f_cadet_arag(i,j,k) + cobalt%f_cadet_calc(i,j,k)) - &
               cobalt%rplith*cobalt%f_lithdet(i,j,k) - cobalt%rpsio2*cobalt%f_sidet(i,j,k) )
          ! Nitrate reduction to nitrite
          cobalt%jremin_ndet_denit0(i,j,k) = cobalt%gamma_ndet_denit0 * cobalt%expkreminT(i,j,k) * &
               exp( - cobalt%f_o2(i,j,k) / cobalt%k_o2_denit0 ) * &
               cobalt%f_no3(i,j,k) / (cobalt%k_no3_denit + cobalt%f_no3(i,j,k))* &
               max(0.0, cobalt%f_ndet(i,j,k) - &
               cobalt%rpcaco3*(cobalt%f_cadet_arag(i,j,k) + cobalt%f_cadet_calc(i,j,k)) - &
          ...
          cobalt%jno3denit_wc(i,j,k) = cobalt%jno3denit_wc(i,j,k) + cobalt%jremin_ndet_denit1(i,j,k) * &
               cobalt%n_2_n_denit * ( 5.0 / 4.0 )
       ...
       ! Augment total ndet consumption due to anammox
       ...
       cobalt%jprod_po4(i,j,k) = cobalt%jprod_po4(i,j,k) + cobalt%jremin_pdet(i,j,k) + cobalt%jremin_pdet_fast(i,j,k) ! } YZ
       ...
       cobalt%jremin_fedet(i,j,k) = cobalt%jremin_fedet(i,j,k) + &
            (cobalt%jremin_ndet_denit0(i,j,k) / (exp( - cobalt%f_o2(i,j,k) + epsln) / cobalt%k_o2_denit0 ) + &
            cobalt%jremin_ndet_denit2(i,j,k) / (exp( - cobalt%f_o2(i,j,k) + epsln) / cobalt%k_o2_denit2 )) / &
    ...
    do k=1,nk ; do j=jsc,jec ; do i=isc,iec  !{
       ...
       if (cobalt%f_o2(i,j,k) .gt. cobalt%o2_min) then  !{
          ...
          cobalt%jprod_nh4(i,j,k) = cobalt%jprod_nh4(i,j,k) + cobalt%jremin_ndet(i,j,k) + cobalt%jremin_ndet_fast(i,j,k) 
          cobalt%jo2resp_wc(i,j,k) = cobalt%jo2resp_wc(i,j,k) + &
               (cobalt%jremin_ndet(i,j,k) + cobalt%jremin_ndet_fast(i,j,k)) * cobalt%o2_2_nh4 ! } YZ
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
       ...
       if (cobalt%feprime(i,j,k).lt.cobalt%fe_sol(i,j,k)) then
         cobalt%jfe_ads(i,j,k) = cobalt%alpha_fescav*cobalt%feprime(i,j,k) + &
                                 cobalt%beta_fescav*cobalt%feprime(i,j,k)*(cobalt%f_ndet(i,j,k)+cobalt%f_ndet_fast(i,j,k)) ! YZ: fast-sinking
       else
    ...
    ! YZ: N2O_module, 13/06/2025 {
    !
    ! 4.6: N2O production and consumption
    !
    ! N2O is produced by nitrification and nitrate reduction to N2O during denitrification, and consumed
    ! by N2O reduction to N2 during denitrification.
    ! N2O consumption by denitrification is proportional to their fraction in total N2O concentrations.
    !
    if (do_n2o) then !{
       do k = 1, nk ; do j = jsc, jec ; do i = isc, iec   !{
         ! N2O production via nitrification as a byproduct
          if (cobalt%f_o2(i,j,k) .gt. cobalt%o2_min_nit ) then !{
          ...
          if (do_n2o_decomp) then !{
          cobalt%jsink_n2o_nit(i,j,k) = cobalt%f_n2o_nit(i,j,k) / (cobalt%f_n2o(i,j,k) + epsln) * cobalt%jsink_n2o(i,j,k)
          cobalt%jsink_n2o_denit(i,j,k) = cobalt%f_n2o_denit(i,j,k) / (cobalt%f_n2o(i,j,k) + epsln) * cobalt%jsink_n2o(i,j,k)
          cobalt%jsink_n2o_atm(i,j,k) = cobalt%f_n2o_atm(i,j,k) / (cobalt%f_n2o(i,j,k) + epsln) * cobalt%jsink_n2o(i,j,k)
          cobalt%jsink_n2o_sed(i,j,k) = cobalt%f_n2o_sed(i,j,k) / (cobalt%f_n2o(i,j,k) + epsln) * cobalt%jsink_n2o(i,j,k)
          endif !}

          ! YZ: N2O_diag, 06/12/2025 {
          ! Calculate diagnostic N2O production based on oxygen consumption
          ! 1) Similar to commonly used formulation, N2O production in the euphotic zone is set to be zero
          ! 2) the formulation of oxygen dependence follows Suntharalingam et al. (2000) with oxygen threshold of 1 umol/kg 
          ! 3) N2O consumption is calculated when oxygen is below o2_max_diag, equivalent to all N2O production as the upper bound
          if (do_n2o_diag) then !{
             if (cobalt%irr_inst(i,j,k) .lt. (sfc_irrad(i,j)*0.01)) then !{   
                ! calculate the functional dependence of N2O yield on oxygen level
                if (cobalt%f_o2(i,j,k) .lt. cobalt%o2_offset_diag) then !{ 
                   fo2_dependence = cobalt%f_o2(i,j,k)/cobalt%o2_offset_diag
                else
                   fo2_dependence = exp( - cobalt%lamda_n2o_diag * (cobalt%f_o2(i,j,k) - cobalt%o2_offset_diag) / &
                           cobalt%o2_offset_diag)
                endif !}
                ! calculate N2O production via nitrification in oxygenated conditions
                cobalt%jprod_n2o_diag_nit(i,j,k) = cobalt%alpha_n2o_diag * cobalt%jo2resp_wc(i,j,k)
                ! calculate enhanced N2O production in low-oxygen conditions
                cobalt%jprod_n2o_diag_lowo2(i,j,k) = cobalt%beta_n2o_diag * fo2_dependence * cobalt%jo2resp_wc(i,j,k)
                cobalt%jprod_n2o_diag(i,j,k) = cobalt%jprod_n2o_diag_nit(i,j,k) + cobalt%jprod_n2o_diag_lowo2(i,j,k)
                ! calculate N2O loss in anoxic conditions
                ! the consumption of n2o_diag_nit and n2o_diag_lowo2 are proportional to their concentrations
                if (cobalt%f_o2(i,j,k) .lt. cobalt%o2_max_diag) then !{
                   cobalt%jsink_n2o_diag(i,j,k) = cobalt%jprod_n2o_diag(i,j,k)
                   cobalt%jsink_n2o_diag_nit(i,j,k) = cobalt%f_n2o_diag_nit(i,j,k)/(cobalt%f_n2o_diag(i,j,k) + epsln) * &
                           cobalt%jsink_n2o_diag(i,j,k)
                   cobalt%jsink_n2o_diag_lowo2(i,j,k) = cobalt%f_n2o_diag_lowo2(i,j,k)/(cobalt%f_n2o_diag(i,j,k) + epsln) * &
                           cobalt%jsink_n2o_diag(i,j,k)
                else
                   cobalt%jsink_n2o_diag(i,j,k) = 0.0
                   cobalt%jsink_n2o_diag_nit(i,j,k) = 0.0
...
!
    ...
    do j = jsc, jec; do i = isc, iec  !{
       if (grid_kmt(i,j) .gt. 0) then !{
          ...
          ! YZ: N2O_module, 13/06/2025 {
          if (do_n2o) then !{
            cobalt%btm_n2o(i,j) = 0.0
            if (do_n2o_decomp) then !{
            cobalt%btm_n2o_nit(i,j) = 0.0
            cobalt%btm_n2o_denit(i,j) = 0.0
            cobalt%btm_n2o_atm(i,j) = 0.0
            cobalt%btm_n2o_sed(i,j) = 0.0
            endif !}
          endif !} ! } YZ
          k_bot(i,j) = 0
          ...
          do k = grid_kmt(i,j),1,-1   !{
            ...
            if (rho_dzt_bot(i,j).lt.(cobalt%Rho_0*cobalt%bottom_thickness)) then
              ...
              ! YZ: N2O_module, 13/06/2025 {
              if (do_n2o) then !{
                cobalt%btm_n2o(i,j) = cobalt%btm_n2o(i,j) + cobalt%f_n2o(i,j,k)*rho_dzt(i,j,k)
                if (do_n2o_decomp) then !{
                cobalt%btm_n2o_nit(i,j) = cobalt%btm_n2o_nit(i,j) + cobalt%f_n2o_nit(i,j,k)*rho_dzt(i,j,k)
                cobalt%btm_n2o_denit(i,j) = cobalt%btm_n2o_denit(i,j) + cobalt%f_n2o_denit(i,j,k)*rho_dzt(i,j,k)
                cobalt%btm_n2o_atm(i,j) = cobalt%btm_n2o_atm(i,j) + cobalt%f_n2o_atm(i,j,k)*rho_dzt(i,j,k)
                cobalt%btm_n2o_sed(i,j) = cobalt%btm_n2o_sed(i,j) + cobalt%f_n2o_sed(i,j,k)*rho_dzt(i,j,k)
                endif !}
              endif !} ! } YZ
            endif
          enddo
          ...
          ! YZ: N2O_module, 13/06/2025 {
          if (do_n2o) then !{
            cobalt%btm_n2o(i,j)=cobalt%btm_n2o(i,j)-cobalt%f_n2o(i,j,k_bot(i,j))*drho_dzt
            if (do_n2o_decomp) then !{
            cobalt%btm_n2o_nit(i,j)=cobalt%btm_n2o_nit(i,j)-cobalt%f_n2o_nit(i,j,k_bot(i,j))*drho_dzt
            cobalt%btm_n2o_denit(i,j)=cobalt%btm_n2o_denit(i,j)-cobalt%f_n2o_denit(i,j,k_bot(i,j))*drho_dzt
            cobalt%btm_n2o_atm(i,j)=cobalt%btm_n2o_atm(i,j)-cobalt%f_n2o_atm(i,j,k_bot(i,j))*drho_dzt
            cobalt%btm_n2o_sed(i,j)=cobalt%btm_n2o_sed(i,j)-cobalt%f_n2o_sed(i,j,k_bot(i,j))*drho_dzt
            endif !}
          endif !} ! } YZ
          ...
          ! YZ: N2O_module, 13/06/2025 {
          if (do_n2o) then !{
            cobalt%btm_n2o(i,j)=cobalt%btm_n2o(i,j)/(cobalt%bottom_thickness*cobalt%Rho_0)
            if (do_n2o_decomp) then !{
            cobalt%btm_n2o_nit(i,j)=cobalt%btm_n2o_nit(i,j)/(cobalt%bottom_thickness*cobalt%Rho_0)
            cobalt%btm_n2o_denit(i,j)=cobalt%btm_n2o_denit(i,j)/(cobalt%bottom_thickness*cobalt%Rho_0)
            cobalt%btm_n2o_atm(i,j)=cobalt%btm_n2o_atm(i,j)/(cobalt%bottom_thickness*cobalt%Rho_0)
            cobalt%btm_n2o_sed(i,j)=cobalt%btm_n2o_sed(i,j)/(cobalt%bottom_thickness*cobalt%Rho_0)
            endif !}
          endif !} ! } YZ
          ...
          do k = 1, nk !{
            if (cobalt%fe_coast == 0.0) then
              ...
              cobalt%jfe_coast(i,j,k) = cobalt%fe_coast*dzt(i,j,k)*mask_coast(i,j)*grid_tmask(i,j,k)* &
                ...
                cobalt%f_ndet_fast(i,j,k)*cobalt%wsink_fast + & ! YZ: fast-sinking, 07/07/2025
          ...
          if (do_n2o) then !{
             ...
             cobalt%b_n2o_atm(i,j) = 0.0
             cobalt%b_n2o_sed(i,j) = - cobalt%fno3denit_sed(i,j) * cobalt%n2o_2_n_sed
             endif !}
             ! YZ: N2O_diag, 06/12/2025 {
             if (do_n2o_diag) then !{
             cobalt%b_n2o_diag(i,j) = 0.0
             cobalt%b_n2o_diag_nit(i,j) = 0.0
             cobalt%b_n2o_diag_lowo2(i,j) = 0.0
             endif !} ! } YZ
          endif !} ! } YZ
    ...
    if (do_n2o) then !{
       ...
       call g_tracer_set_values(tracer_list,'n2o_atm', 'btf', cobalt%b_n2o_atm,isd,jsd)
       call g_tracer_set_values(tracer_list,'n2o_sed', 'btf', cobalt%b_n2o_sed,isd,jsd)
       endif !}
       ! YZ: N2O_diag, 06/12/2025 {
       if (do_n2o_diag) then !{
       call g_tracer_set_values(tracer_list,'n2o_diag', 'btf', cobalt%b_n2o_diag,isd,jsd)
       call g_tracer_set_values(tracer_list,'n2o_diag_nit', 'btf', cobalt%b_n2o_diag_nit,isd,jsd)
       call g_tracer_set_values(tracer_list,'n2o_diag_lowo2', 'btf', cobalt%b_n2o_diag_lowo2,isd,jsd)
       endif !} ! } YZ
    endif !}  ! } YZ
!
...
!
    ...
    if (do_n2o) then !{
       ...
       call g_tracer_get_pointer(tracer_list,'n2o_atm','field',cobalt%p_n2o_atm)
       call g_tracer_get_pointer(tracer_list,'n2o_sed','field',cobalt%p_n2o_sed)
       endif !}
       ! YZ: N2O_diag, 06/12/2025 {
       if (do_n2o_diag) then !{
       call g_tracer_get_pointer(tracer_list,'n2o_diag','field',cobalt%p_n2o_diag)
       call g_tracer_get_pointer(tracer_list,'n2o_diag_nit','field',cobalt%p_n2o_diag_nit)
       call g_tracer_get_pointer(tracer_list,'n2o_diag_lowo2','field',cobalt%p_n2o_diag_lowo2)
       endif !} ! } YZ
    endif !} ! } YZ
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
         pre_totn(i,j,k) = (cobalt%p_no3(i,j,k,tau) + cobalt%p_nh4(i,j,k,tau) + &
                    ...
                    cobalt%p_ndet_fast(i,j,k,tau) + & ! YZ: fast-sinking, 07/07/2025
         ...
         ! YZ: N2O_module, 13/06/2025 {
         ! N loss via N2O production by denitrification has been calculated in jno3denit_wc,
         ! only N loss via N2O production by nitrification as a byproduct should be accounted here
         if (do_n2o) then !{
            net_srcn(i,j,k) = net_srcn(i,j,k) - cobalt%jprod_n2o_nit(i,j,k) * 2.0 *dt*grid_tmask(i,j,k)
         endif !} ! } YZ
         ...
         pre_totp(i,j,k) = (cobalt%p_po4(i,j,k,tau) + cobalt%p_pdi(i,j,k,tau) + &
                    ...
                    cobalt%p_pdet_fast(i,j,k,tau) + & ! YZ: fast-sinking, 07/07/2025
...
!
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       ...
       ! YZ: N2O_module, 13/06/2025 {
       ! account for N source for N2O production via nitrification from NH4 instead of NO3
       if (do_n2o) then !{
          cobalt%jnh4(i,j,k) = cobalt%jnh4(i,j,k) - cobalt%jprod_n2o_nit(i,j,k) * 2.0
       endif !} ! } YZ
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       ...
       cobalt%jpdet_fast(i,j,k) = cobalt%jprod_pdet_fast(i,j,k) - cobalt%jremin_pdet_fast(i,j,k) ! YZ: fast-sinking, 07/07/2025
       cobalt%p_pdet(i,j,k,tau) = cobalt%p_pdet(i,j,k,tau) + cobalt%jpdet(i,j,k)*dt*grid_tmask(i,j,k)
       cobalt%p_pdet_fast(i,j,k,tau) = cobalt%p_pdet_fast(i,j,k,tau) + cobalt%jpdet_fast(i,j,k)*dt*grid_tmask(i,j,k) ! YZ: fast-sinking
       !
       ! Sidet
       !
    ...
    do k = 1, nk ; do j =jsc, jec ; do i = isc, iec   !{
       ...
       cobalt%jn2o_atm(i,j,k) = - cobalt%jsink_n2o_atm(i,j,k)
       cobalt%jn2o_sed(i,j,k) = - cobalt%jsink_n2o_sed(i,j,k)

       cobalt%p_n2o_nit(i,j,k,tau) = cobalt%p_n2o_nit(i,j,k,tau) + cobalt%jn2o_nit(i,j,k) * dt * grid_tmask(i,j,k)
       cobalt%p_n2o_denit(i,j,k,tau) = cobalt%p_n2o_denit(i,j,k,tau) + cobalt%jn2o_denit(i,j,k) * dt * grid_tmask(i,j,k)
       cobalt%p_n2o_atm(i,j,k,tau) = cobalt%p_n2o_atm(i,j,k,tau) + cobalt%jn2o_atm(i,j,k) * dt * grid_tmask(i,j,k)
       cobalt%p_n2o_sed(i,j,k,tau) = cobalt%p_n2o_sed(i,j,k,tau) + cobalt%jn2o_sed(i,j,k) * dt * grid_tmask(i,j,k)
       endif !}

       ! YZ: N2O_diag, 06/12/2025 {
       if (do_n2o_diag) then !{
       cobalt%jn2o_diag(i,j,k) = cobalt%jprod_n2o_diag(i,j,k) - cobalt%jsink_n2o_diag(i,j,k)
       cobalt%jn2o_diag_nit(i,j,k) = cobalt%jprod_n2o_diag_nit(i,j,k) - cobalt%jsink_n2o_diag_nit(i,j,k)
       cobalt%jn2o_diag_lowo2(i,j,k) = cobalt%jprod_n2o_diag_lowo2(i,j,k) - cobalt%jsink_n2o_diag_lowo2(i,j,k)
       
       cobalt%p_n2o_diag(i,j,k,tau) = cobalt%p_n2o_diag(i,j,k,tau) + cobalt%jn2o_diag(i,j,k) * dt * grid_tmask(i,j,k)
       cobalt%p_n2o_diag_nit(i,j,k,tau) = cobalt%p_n2o_diag_nit(i,j,k,tau) + cobalt%jn2o_diag_nit(i,j,k) * dt * grid_tmask(i,j,k)
       cobalt%p_n2o_diag_lowo2(i,j,k,tau) = cobalt%p_n2o_diag_lowo2(i,j,k,tau) + cobalt%jn2o_diag_lowo2(i,j,k) * dt * grid_tmask(i,j,k)
       endif !} ! } YZ
    enddo; enddo ; enddo  !} i,j,k
    endif !} ! } YZ
    !
    ...
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       ...
       ! YZ: N2O_module, 13/06/2025 {
       ! Modify the alkalinity changes due to denitrification
       if (do_n2o) then !{
          cobalt%jalk(i,j,k) = cobalt%jalk(i,j,k) + (cobalt%alk_2_n2o_denit1 - cobalt%alk_2_n_denit * 2.0) * &
              cobalt%jprod_n2o_denit(i,j,k) + cobalt%alk_2_n2o_denit2*cobalt%jsink_n2o(i,j,k)
       endif !} ! } YZ
...
!
    ...
    ! YZ: N2O_module, 13/06/2025 {
    ! Calculate the N2O saturation using the relationships of Weiss and Price (1980).
    if (do_n2o) then !{
    log100 = log(100.0)
    do k = 1, nk  ; do j = jsc, jec ; do i = isc, iec !{
       sal = min(42.0,max(0.0,Salt(i,j,k)))
       tkn = 273.15 + min(40.0,max(0.0,Temp(i,j,k)))
       tkn100 = tkn / 100.0
       tkn1002 = tkn100 * tkn100
       logtkn = log(tkn)

       !The atmospheric code needs solubilities in units of mol/m3/atm
       cobalt%n2osat(i,j,k) = 1000.0 * grid_tmask(i,j,1) * & !convert from mol/l to mol m-3
            exp( cobalt%a_1_n2o + cobalt%a_2_n2o/tkn100 + cobalt%a_3_n2o*(logtkn-log100) + &
            (cobalt%b_1_n2o + cobalt%b_2_n2o*tkn100 + cobalt%b_3_n2o*tkn1002)*sal)
    enddo; enddo ; enddo  !} i,j,k
    endif !} ! } YZ
    ...
    cobalt%tot_layer_int_p(:,:,:) = (cobalt%p_po4(:,:,:,tau) + cobalt%p_pdi(:,:,:,tau) + cobalt%p_plg(:,:,:,tau) + &
         ...
         cobalt%p_srdop(:,:,:,tau) + cobalt%p_pdet(:,:,:,tau) + cobalt%p_pdet_fast(:,:,:,tau) + & ! YZ: fast-sinking
    ...
    if (do_n2o) then !{
       ...
       cobalt%tot_layer_int_n2o_atm(:,:,:) = cobalt%p_n2o_atm(:,:,:,tau)*rho_dzt(:,:,:)
       cobalt%tot_layer_int_n2o_sed(:,:,:) = cobalt%p_n2o_sed(:,:,:,tau)*rho_dzt(:,:,:)
       endif !}
       ! YZ: N2O_diag, 06/12/2025 {
       if (do_n2o_diag) then !{
       cobalt%tot_layer_int_n2o_diag(:,:,:) = cobalt%p_n2o_diag(:,:,:,tau)*rho_dzt(:,:,:)
       cobalt%tot_layer_int_n2o_diag_nit(:,:,:) = cobalt%p_n2o_diag_nit(:,:,:,tau)*rho_dzt(:,:,:)
       cobalt%tot_layer_int_n2o_diag_lowo2(:,:,:) = cobalt%p_n2o_diag_lowo2(:,:,:,tau)*rho_dzt(:,:,:)
       endif !} ! } YZ
    endif !} ! } YZ
    ...
    do j = jsc, jec ; do i = isc, iec !{
       ...
       if (do_n2o) then !{
          ...
          cobalt%wc_vert_int_jsink_n2o_sed(i,j) = 0.0
          endif !}
          ! YZ: N2O_diag, 06/12/2025 {
          if (do_n2o_diag) then !{
          cobalt%wc_vert_int_n2o_diag(i,j) = 0.0
          cobalt%wc_vert_int_n2o_diag_nit(i,j) = 0.0
          cobalt%wc_vert_int_n2o_diag_lowo2(i,j) = 0.0
          cobalt%wc_vert_int_jn2o_diag(i,j) = 0.0
          cobalt%wc_vert_int_jn2o_diag_nit(i,j) = 0.0
          cobalt%wc_vert_int_jn2o_diag_lowo2(i,j) = 0.0
          cobalt%wc_vert_int_jprod_n2o_diag(i,j) = 0.0
          cobalt%wc_vert_int_jprod_n2o_diag_nit(i,j) = 0.0
          cobalt%wc_vert_int_jprod_n2o_diag_lowo2(i,j) = 0.0
          cobalt%wc_vert_int_jsink_n2o_diag(i,j) = 0.0
          cobalt%wc_vert_int_jsink_n2o_diag_nit(i,j) = 0.0
          cobalt%wc_vert_int_jsink_n2o_diag_lowo2(i,j) = 0.0
          endif !} ! } YZ
       endif !} ! } YZ
    enddo; enddo !} i,j
    do j = jsc, jec ; do i = isc, iec ; do k = 1, nk  !{
          ...
          if (do_n2o) then !{
             ...
             cobalt%wc_vert_int_n2o_denit(i,j) = cobalt%wc_vert_int_n2o_denit(i,j) + &
                cobalt%tot_layer_int_n2o_denit(i,j,k)*grid_tmask(i,j,k)
             cobalt%wc_vert_int_n2o_atm(i,j) = cobalt%wc_vert_int_n2o_atm(i,j) + &
                cobalt%tot_layer_int_n2o_atm(i,j,k)*grid_tmask(i,j,k)
             cobalt%wc_vert_int_n2o_sed(i,j) = cobalt%wc_vert_int_n2o_sed(i,j) + &
                cobalt%tot_layer_int_n2o_sed(i,j,k)*grid_tmask(i,j,k)
             endif !}
             ! YZ: N2O_diag, 06/12/2025 {
             if (do_n2o_diag) then !{
             cobalt%wc_vert_int_n2o_diag(i,j) = cobalt%wc_vert_int_n2o_diag(i,j) + &
                cobalt%tot_layer_int_n2o_diag(i,j,k)*grid_tmask(i,j,k)
             cobalt%wc_vert_int_n2o_diag_nit(i,j) = cobalt%wc_vert_int_n2o_diag_nit(i,j) + &
                cobalt%tot_layer_int_n2o_diag_nit(i,j,k)*grid_tmask(i,j,k)
             cobalt%wc_vert_int_n2o_diag_lowo2(i,j) = cobalt%wc_vert_int_n2o_diag_lowo2(i,j) + &
                cobalt%tot_layer_int_n2o_diag_lowo2(i,j,k)*grid_tmask(i,j,k)
             endif !} ! } YZ
          endif !} ! } YZ
          ...
          if (do_n2o) then !{
             ...
             cobalt%wc_vert_int_jsink_n2o_denit(i,j) = cobalt%wc_vert_int_jsink_n2o_denit(i,j) + &
                cobalt%jsink_n2o_denit(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jsink_n2o_atm(i,j) = cobalt%wc_vert_int_jsink_n2o_atm(i,j) + &
                cobalt%jsink_n2o_atm(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jsink_n2o_sed(i,j) = cobalt%wc_vert_int_jsink_n2o_sed(i,j) + &
                cobalt%jsink_n2o_sed(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             endif !}
             ! YZ: N2O_diag, 06/12/2025 {
             if (do_n2o_diag) then !{
             cobalt%wc_vert_int_jn2o_diag(i,j) = cobalt%wc_vert_int_jn2o_diag(i,j) + &
                cobalt%jn2o_diag(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jn2o_diag_nit(i,j) = cobalt%wc_vert_int_jn2o_diag_nit(i,j) + &
                cobalt%jn2o_diag_nit(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jn2o_diag_lowo2(i,j) = cobalt%wc_vert_int_jn2o_diag_lowo2(i,j) + &
                cobalt%jn2o_diag_lowo2(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jprod_n2o_diag(i,j) = cobalt%wc_vert_int_jprod_n2o_diag(i,j) + &
                cobalt%jprod_n2o_diag(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jprod_n2o_diag_nit(i,j) = cobalt%wc_vert_int_jprod_n2o_diag_nit(i,j) + &
                cobalt%jprod_n2o_diag_nit(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jprod_n2o_diag_lowo2(i,j) = cobalt%wc_vert_int_jprod_n2o_diag_lowo2(i,j) + &
                cobalt%jprod_n2o_diag_lowo2(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jsink_n2o_diag(i,j) = cobalt%wc_vert_int_jsink_n2o_diag(i,j) + &
                cobalt%jsink_n2o_diag(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jsink_n2o_diag_nit(i,j) = cobalt%wc_vert_int_jsink_n2o_diag_nit(i,j) + &
                cobalt%jsink_n2o_diag_nit(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             cobalt%wc_vert_int_jsink_n2o_diag_lowo2(i,j) = cobalt%wc_vert_int_jsink_n2o_diag_lowo2(i,j) + &
                cobalt%jsink_n2o_diag_lowo2(i,j,k)*rho_dzt(i,j,k) * grid_tmask(i,j,k)
             endif !} ! } YZ
          endif !} ! } YZ
    ...
    do j = jsc, jec ; do i = isc, iec ; do k = 1, nk  !{
       ...
       if (do_n2o) then !{
         ...
         cobalt%jn2o_atm_plus_btm(i,j,k)   = cobalt%jn2o_atm(i,j,k)
         cobalt%jn2o_sed_plus_btm(i,j,k)   = cobalt%jn2o_sed(i,j,k)
         endif !}
         ! YZ: N2O_diag, 06/12/2025 {
         if (do_n2o_diag) then !{
         cobalt%jn2o_diag_plus_btm(i,j,k)   = cobalt%jn2o_diag(i,j,k)
         cobalt%jn2o_diag_nit_plus_btm(i,j,k)   = cobalt%jn2o_diag_nit(i,j,k)
         cobalt%jn2o_diag_lowo2_plus_btm(i,j,k)   = cobalt%jn2o_diag_lowo2(i,j,k)
         endif !} ! } YZ
       endif !} ! } YZ
    enddo; enddo; enddo  !} i,j,k
      ...
      if (k .gt. 0) then !{
        do k = grid_kmt(i,j),k_bot(i,j),-1   !{
          ...
          if (do_n2o) then !{
            ...
            cobalt%jn2o_atm_plus_btm(i,j,k) = cobalt%jn2o_atm(i,j,k) - cobalt%b_n2o_atm(i,j)/rho_dzt_bot(i,j)
            cobalt%jn2o_sed_plus_btm(i,j,k) = cobalt%jn2o_sed(i,j,k) - cobalt%b_n2o_sed(i,j)/rho_dzt_bot(i,j)
            endif !}
            ! YZ: N2O_diag, 06/12/2025 {
            if (do_n2o_diag) then !{
            cobalt%jn2o_diag_plus_btm(i,j,k) = cobalt%jn2o_diag(i,j,k) - cobalt%b_n2o_diag(i,j)/rho_dzt_bot(i,j)
            cobalt%jn2o_diag_nit_plus_btm(i,j,k) = cobalt%jn2o_diag_nit(i,j,k) - cobalt%b_n2o_diag_nit(i,j)/rho_dzt_bot(i,j)
            cobalt%jn2o_diag_lowo2_plus_btm(i,j,k) = cobalt%jn2o_diag_lowo2(i,j,k) - cobalt%b_n2o_diag_lowo2(i,j)/rho_dzt_bot(i,j)
            endif !} ! } YZ
          endif !} ! } YZ
        enddo
      endif
...
!****************************************************************************************************
    ...
    do j = jsc, jec ; do i = isc, iec !{
       ...
       cobalt%jremin_ndet_fast_100(i,j) = cobalt%jremin_ndet_fast(i,j,1) * rho_dzt(i,j,1) ! YZ: fast-sinking, 07/07/2025
    enddo; enddo !} i,j
    ...
    do j = jsc, jec ; do i = isc, iec ; !{
       ...
       do k = 2, grid_kmt(i,j)  !{
          if (rho_dzt_100(i,j) .lt. cobalt%Rho_0 * 100.0) then
             ...
             cobalt%jremin_ndet_fast_100(i,j) = cobalt%jremin_ndet_fast_100(i,j) + cobalt%jremin_ndet_fast(i,j,k) * rho_dzt(i,j,k) ! YZ, fast-sinking
          endif
       enddo  !} k
       ...
       if (k_100 .gt. 1 .and. k_100 .lt. grid_kmt(i,j)) then
          ...
          do n = 1, NUM_PHYTO !{
           ...
           cobalt%jremin_ndet_fast_100(i,j) = cobalt%jremin_ndet_fast_100(i,j) + cobalt%jremin_ndet_fast(i,j,k_100)* &
                drho_dzt ! YZ: fast-sinking, 07/07/2025
       endif
...
!    call g_tracer_get_values(tracer_list,'srdop','runoff_tracer_flux',cobalt%runoff_flux_srdop,isd,jsd)
    ...
    if (do_n2o) then !{
      ...
      call g_tracer_get_values(tracer_list,'n2o_atm','stf_gas',cobalt%stf_gas_n2o_atm,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_sed','stf_gas',cobalt%stf_gas_n2o_sed,isd,jsd)

      call g_tracer_get_values(tracer_list,'n2o_nit','deltap',cobalt%deltap_n2o_nit,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_denit','deltap',cobalt%deltap_n2o_denit,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_atm','deltap',cobalt%deltap_n2o_atm,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_sed','deltap',cobalt%deltap_n2o_sed,isd,jsd)
      endif !}
    endif !} ! } YZ
...
!==============================================================================================================
  ...
  subroutine generic_COBALT_set_boundary_values(tracer_list,SST,SSS,rho,ilb,jlb,tau,dzt,model_time)
    ...
    real    :: sal,ST,o2_saturation,n2o_saturation ! YZ: N2O_module, 13/06/2025
    ...
    real, dimension(:,:,:,:), pointer :: n2o_field,n2o_nit_field,n2o_denit_field,n2o_atm_field,n2o_sed_field ! YZ: N2O_module, 13/06/2025 {
    ...
    real, dimension(:,:), ALLOCATABLE :: n2o_diag_sc_no,n2o_diag_nit_sc_no,n2o_diag_lowo2_sc_no ! } YZ
    real :: pka_nh3,tr,ltr
    ...
    if (do_n2o) then !{
     ...
     if (do_n2o_decomp) then !{
      ...
      call g_tracer_get_pointer(tracer_list,'n2o_atm' ,'field',  n2o_atm_field)
      call g_tracer_get_pointer(tracer_list,'n2o_sed' ,'field',  n2o_sed_field)
      endif !}
      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      call g_tracer_get_pointer(tracer_list,'n2o_diag' ,'field',  n2o_diag_field)
      call g_tracer_get_pointer(tracer_list,'n2o_diag_nit' ,'field',  n2o_diag_nit_field)
      call g_tracer_get_pointer(tracer_list,'n2o_diag_lowo2' ,'field',  n2o_diag_lowo2_field)
      endif !} ! } YZ
    endif !} ! } YZ
    ...
    if (do_n2o) then !{
      ...
      allocate(n2o_atm_alpha(isd:ied, jsd:jed)); n2o_atm_alpha=0.0
      allocate(n2o_sed_alpha(isd:ied, jsd:jed)); n2o_sed_alpha=0.0

      allocate(n2o_nit_csurf(isd:ied, jsd:jed)); n2o_nit_csurf=0.0
      allocate(n2o_denit_csurf(isd:ied, jsd:jed)); n2o_denit_csurf=0.0
      allocate(n2o_atm_csurf(isd:ied, jsd:jed)); n2o_atm_csurf=0.0
      allocate(n2o_sed_csurf(isd:ied, jsd:jed)); n2o_sed_csurf=0.0

      allocate(n2o_nit_sc_no(isd:ied, jsd:jed)); n2o_nit_sc_no=0.0
      allocate(n2o_denit_sc_no(isd:ied, jsd:jed)); n2o_denit_sc_no=0.0
      allocate(n2o_atm_sc_no(isd:ied, jsd:jed)); n2o_atm_sc_no=0.0
      allocate(n2o_sed_sc_no(isd:ied, jsd:jed)); n2o_sed_sc_no=0.0
      endif !}
      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      allocate(n2o_diag_alpha(isd:ied, jsd:jed)); n2o_diag_alpha=0.0
      allocate(n2o_diag_csurf(isd:ied, jsd:jed)); n2o_diag_csurf=0.0
      allocate(n2o_diag_sc_no(isd:ied, jsd:jed)); n2o_diag_sc_no=0.0

      allocate(n2o_diag_nit_alpha(isd:ied, jsd:jed)); n2o_diag_nit_alpha=0.0
      allocate(n2o_diag_nit_csurf(isd:ied, jsd:jed)); n2o_diag_nit_csurf=0.0
      allocate(n2o_diag_nit_sc_no(isd:ied, jsd:jed)); n2o_diag_nit_sc_no=0.0

      allocate(n2o_diag_lowo2_alpha(isd:ied, jsd:jed)); n2o_diag_lowo2_alpha=0.0
      allocate(n2o_diag_lowo2_csurf(isd:ied, jsd:jed)); n2o_diag_lowo2_csurf=0.0
      allocate(n2o_diag_lowo2_sc_no(isd:ied, jsd:jed)); n2o_diag_lowo2_sc_no=0.0
      endif !} ! } YZ
    endif !} ! } YZ
    ...
    if (do_n2o) then !{
      ...
      call g_tracer_get_values(tracer_list,'n2o_atm','alpha', n2o_atm_alpha ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_sed','alpha', n2o_sed_alpha ,isd,jsd)

      call g_tracer_get_values(tracer_list,'n2o_nit','csurf', n2o_nit_csurf ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_denit','csurf', n2o_denit_csurf ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_atm','csurf', n2o_atm_csurf ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_sed','csurf', n2o_sed_csurf ,isd,jsd)
      endif !}

      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      call g_tracer_get_values(tracer_list,'n2o_diag','alpha', n2o_diag_alpha ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_diag_nit','alpha', n2o_diag_nit_alpha ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_diag_lowo2','alpha', n2o_diag_lowo2_alpha ,isd,jsd)

      call g_tracer_get_values(tracer_list,'n2o_diag','csurf', n2o_diag_csurf ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_diag_nit','csurf', n2o_diag_nit_csurf ,isd,jsd)
      call g_tracer_get_values(tracer_list,'n2o_diag_lowo2','csurf', n2o_diag_lowo2_csurf ,isd,jsd)
      endif !} ! } YZ
    endif !} ! } YZ
...
!
       ...
       !  The formula used is from page 351, eq (12).
       !
       !  n2o_saturation is defined between T(freezing) <= T <= 40 deg C and
       !                               	0 permil <= S <= 42 permil
       ! check value: T = 10 deg C, S = 35 permil,
       !          	n2o_saturation = 0.0330465 mol l-1 atm-1
       !---------------------------------------------------------------------
       !
       log100 = log(100.0)
       sal = min(42.0,max(0.0,sal))
       tkn = 273.15 + min(40.0,max(0.0,ST))
       tkn100 = tkn / 100.0
       tkn1002 = tkn100  * tkn100
       logtkn = log(tkn)

       !The atmospheric code needs solubilities in units of mol/m3/atm
       n2o_saturation = 1000.0 * grid_tmask(i,j,1) *  & !convert from mol/l to mol m-3
            exp( cobalt%a_1_n2o + cobalt%a_2_n2o/tkn100 + cobalt%a_3_n2o*(logtkn-log100) + &
            (cobalt%b_1_n2o + cobalt%b_2_n2o*tkn100 + cobalt%b_3_n2o*tkn1002)*sal)
       ...
       !  formulation presented by Wanninkhof (1992, J. Geophys. Res., 97,
       !  7373-7382).
       !  2018/01/17 jgj  update Schmidt number for N2O to use
       !  Wanninkhof, Limnol. Oceanogr: Methods, 12, 2014, 351-362
       !---------------------------------------------------------------------
       !
       ! In 'ocmip2_generic' atmos_ocean_fluxes.F90 coupler formulation,
       ...
       !
       if (trim(as_param_cobalt) == 'W92') then
         n2o_sc_no(i,j)  = cobalt%a1_n2o + ST * (cobalt%a2_n2o + ST * (cobalt%a3_n2o + ST * cobalt%a4_n2o )) * &
                           grid_tmask(i,j,1)
       else if ((trim(as_param_cobalt) == 'W14') .or. (trim(as_param_cobalt) == 'gfdl_cmip6')) then
         n2o_sc_no(i,j) = cobalt%a1_n2o + ST*(cobalt%a2_n2o + ST*(cobalt%a3_n2o + &
                          ST*(cobalt%a4_n2o + ST*cobalt%a5_n2o))) * grid_tmask(i,j,1)
       endif
       ...
       n2o_denit_sc_no(i,j) = n2o_sc_no(i,j)
       n2o_atm_sc_no(i,j) = n2o_sc_no(i,j)
       n2o_sed_sc_no(i,j) = n2o_sc_no(i,j)

       n2o_nit_alpha(i,j) = n2o_saturation   !nnz: MOM has rho(i,j,1,tau)
       n2o_denit_alpha(i,j) = n2o_saturation !nnz: MOM has rho(i,j,1,tau)
       n2o_atm_alpha(i,j) = n2o_saturation   !nnz: MOM has rho(i,j,1,tau)
       n2o_sed_alpha(i,j) = n2o_saturation   !nnz: MOM has rho(i,j,1,tau)
      
       n2o_nit_csurf(i,j) = n2o_nit_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       n2o_denit_csurf(i,j) = n2o_denit_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       n2o_atm_csurf(i,j) = n2o_atm_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       n2o_sed_csurf(i,j) = n2o_sed_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       endif !}

       ! YZ: N2O_diag, 06/12/2025 {
       if (do_n2o_diag) then !{
       n2o_diag_sc_no(i,j) = n2o_sc_no(i,j)        
       n2o_diag_nit_sc_no(i,j) = n2o_sc_no(i,j)
       n2o_diag_lowo2_sc_no(i,j) = n2o_sc_no(i,j)

       n2o_diag_alpha(i,j) = n2o_saturation   !nnz: MOM has rho(i,j,1,tau)
       n2o_diag_nit_alpha(i,j) = n2o_saturation   !nnz: MOM has rho(i,j,1,tau)
       n2o_diag_lowo2_alpha(i,j) = n2o_saturation   !nnz: MOM has rho(i,j,1,tau)

       n2o_diag_csurf(i,j) = n2o_diag_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       n2o_diag_nit_csurf(i,j) = n2o_diag_nit_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       n2o_diag_lowo2_csurf(i,j) = n2o_diag_lowo2_field(i,j,1,tau) * cobalt%Rho_0  !nnz: MOM has rho(i,j,1,tau)
       endif !} ! } YZ
     endif !} ! } YZ
    ...
    if (do_n2o) then !{
      ...
      call g_tracer_set_values(tracer_list,'n2o_atm', 'alpha',n2o_atm_alpha, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_sed', 'alpha',n2o_sed_alpha, isd,jsd)

      call g_tracer_set_values(tracer_list,'n2o_nit',  'csurf',n2o_nit_csurf, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_denit','csurf',n2o_denit_csurf, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_atm',  'csurf',n2o_atm_csurf, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_sed',  'csurf',n2o_sed_csurf, isd,jsd)

      call g_tracer_set_values(tracer_list,'n2o_nit', 'sc_no',n2o_nit_sc_no, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_denit', 'sc_no',n2o_denit_sc_no, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_atm', 'sc_no',n2o_atm_sc_no, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_sed', 'sc_no',n2o_sed_sc_no, isd,jsd)
      endif !}

      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      call g_tracer_set_values(tracer_list,'n2o_diag', 'alpha',n2o_diag_alpha, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_diag', 'csurf',n2o_diag_csurf, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_diag', 'sc_no',n2o_diag_sc_no, isd,jsd)

      call g_tracer_set_values(tracer_list,'n2o_diag_nit', 'alpha',n2o_diag_nit_alpha, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_diag_nit', 'csurf',n2o_diag_nit_csurf, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_diag_nit', 'sc_no',n2o_diag_nit_sc_no, isd,jsd)

      call g_tracer_set_values(tracer_list,'n2o_diag_lowo2', 'alpha',n2o_diag_lowo2_alpha, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_diag_lowo2', 'csurf',n2o_diag_lowo2_csurf, isd,jsd)
      call g_tracer_set_values(tracer_list,'n2o_diag_lowo2', 'sc_no',n2o_diag_lowo2_sc_no, isd,jsd)
      endif !} ! } YZ
    endif !} ! } YZ
  ...
  subroutine user_allocate_arrays
    ...
    allocate(bact(1)%no3lim(isd:ied,jsd:jed,nk))           ; bact(1)%no3lim          = 0.0  ! YZ: 07/07/2025
    !
    ...
    allocate(cobalt%f_ndet_fast(isd:ied, jsd:jed, 1:nk))  ; cobalt%f_ndet_fast=0.0 ! YZ: fast-sinking, 07/07/2025
    ...
    allocate(cobalt%f_pdet_fast(isd:ied, jsd:jed, 1:nk))  ; cobalt%f_pdet_fast=0.0 ! YZ: fast-sinking, 07/07/2025
    ...
    allocate(cobalt%f_ndet_fast_btf(isd:ied, jsd:jed, 1:nk)); cobalt%f_ndet_fast_btf=0.0 ! YZ: fast-sinking, 07/07/2025
    allocate(cobalt%f_pdet_btf(isd:ied, jsd:jed, 1:nk))   ; cobalt%f_pdet_btf=0.0
    allocate(cobalt%f_pdet_fast_btf(isd:ied, jsd:jed, 1:nk)); cobalt%f_pdet_fast_btf=0.0 ! YZ: fast-sinking, 07/07/2025
    ...
    allocate(cobalt%jndet_fast(isd:ied, jsd:jed, 1:nk))   ; cobalt%jndet_fast=0.0 ! YZ: fast-sinking, 07/07/2025
    ...
    allocate(cobalt%jpdet_fast(isd:ied, jsd:jed, 1:nk))   ; cobalt%jpdet_fast=0.0 ! YZ: fast-sinking, 07/07/2025
    ...
    allocate(cobalt%jprod_ndet_fast(isd:ied, jsd:jed, 1:nk)); cobalt%jprod_ndet_fast=0.0 ! YZ: fast-sinking, 07/07/2025
    allocate(cobalt%jprod_pdet(isd:ied, jsd:jed, 1:nk))   ; cobalt%jprod_pdet=0.0
    allocate(cobalt%jprod_pdet_fast(isd:ied, jsd:jed, 1:nk)); cobalt%jprod_pdet_fast=0.0 ! YZ: fast-sinking, 07/07/2025
    ...
    allocate(cobalt%fpdet_fast_btm(isd:ied, jsd:jed))     ; cobalt%fpdet_fast_btm=0.0 ! YZ: fast-sinking, 07/07/2025
    allocate(cobalt%fndet_btm(isd:ied, jsd:jed))          ; cobalt%fndet_btm=0.0
    allocate(cobalt%fndet_fast_btm(isd:ied, jsd:jed))     ; cobalt%fndet_fast_btm=0.0 ! YZ: fast-sinking, 07/07/2025
...
!==============================================================================================================
   ...
   allocate(cobalt%jremin_ndet_fast_100(isd:ied,jsd:jed))   ; cobalt%jremin_ndet_fast_100 = 0.0 ! YZ: fast-sinking, 07/07/2025
   ...
   allocate(cobalt%f_ndet_fast_100(isd:ied,jsd:jed))        ; cobalt%f_ndet_fast_100 = 0.0 ! YZ: fast-sinking, 07/07/2025
   ...
   allocate(cobalt%fndet_fast_100(isd:ied,jsd:jed))        ; cobalt%fndet_fast_100 = 0.0 ! YZ: fast-sinking, 07/07/2025
   allocate(cobalt%fpdet_100(isd:ied,jsd:jed))             ; cobalt%fpdet_100 = 0.0
   allocate(cobalt%fpdet_fast_100(isd:ied,jsd:jed))        ; cobalt%fpdet_fast_100 = 0.0 ! YZ: fast-sinking, 07/07/2025
...
!      allocate(cobalt%runoff_flux_srdop(isd:ied, jsd:jed));    cobalt%runoff_flux_srdop=0.0
   ...
   ! YZ: N2O_module, 13/06/2025 {
   if (do_n2o) then !{
      allocate(cobalt%f_n2o(isd:ied, jsd:jed, 1:nk));          cobalt%f_n2o=0.0
      allocate(cobalt%jn2o(isd:ied, jsd:jed, 1:nk));           cobalt%jn2o=0.0
      allocate(cobalt%jn2o_plus_btm(isd:ied, jsd:jed, 1:nk));  cobalt%jn2o_plus_btm=0.0
      allocate(cobalt%jprod_n2o_nit(isd:ied, jsd:jed, 1:nk));  cobalt%jprod_n2o_nit=0.0
      allocate(cobalt%jprod_n2o_denit(isd:ied, jsd:jed, 1:nk));cobalt%jprod_n2o_denit=0.0
      allocate(cobalt%jsink_n2o(isd:ied, jsd:jed, 1:nk));      cobalt%jsink_n2o=0.0
      allocate(cobalt%jremin_ndet_denit1(isd:ied, jsd:jed, 1:nk))  ; cobalt%jremin_ndet_denit1=0.0
      allocate(cobalt%jremin_ndet_denit2(isd:ied, jsd:jed, 1:nk))  ; cobalt%jremin_ndet_denit2=0.0
      allocate(cobalt%jremin_ndet_denit0(isd:ied, jsd:jed, 1:nk))  ; cobalt%jremin_ndet_denit0=0.0
      allocate(cobalt%tot_layer_int_n2o(isd:ied, jsd:jed, 1:nk))   ; cobalt%tot_layer_int_n2o=0.0
      allocate(cobalt%b_n2o(isd:ied, jsd:jed));                cobalt%b_n2o=0.0
      allocate(cobalt%btm_n2o(isd:ied,jsd:jed));               cobalt%btm_n2o = 0.0
      allocate(cobalt%n2osat(isd:ied, jsd:jed, 1:nk));         cobalt%n2osat=0.0
      ...
      allocate(cobalt%f_n2o_denit(isd:ied, jsd:jed, 1:nk));    cobalt%f_n2o_denit=0.0
      allocate(cobalt%f_n2o_atm(isd:ied, jsd:jed, 1:nk));      cobalt%f_n2o_atm=0.0
      allocate(cobalt%f_n2o_sed(isd:ied, jsd:jed, 1:nk));      cobalt%f_n2o_sed=0.0
      allocate(cobalt%jn2o_nit(isd:ied, jsd:jed, 1:nk));       cobalt%jn2o_nit=0.0
      allocate(cobalt%jn2o_denit(isd:ied, jsd:jed, 1:nk));     cobalt%jn2o_denit=0.0
      allocate(cobalt%jn2o_atm(isd:ied, jsd:jed, 1:nk));       cobalt%jn2o_atm=0.0
      allocate(cobalt%jn2o_sed(isd:ied, jsd:jed, 1:nk));       cobalt%jn2o_sed=0.0
      allocate(cobalt%jn2o_nit_plus_btm(isd:ied, jsd:jed, 1:nk));       cobalt%jn2o_nit_plus_btm=0.0
      allocate(cobalt%jn2o_denit_plus_btm(isd:ied, jsd:jed, 1:nk));     cobalt%jn2o_denit_plus_btm=0.0
      allocate(cobalt%jn2o_atm_plus_btm(isd:ied, jsd:jed, 1:nk));       cobalt%jn2o_atm_plus_btm=0.0
      allocate(cobalt%jn2o_sed_plus_btm(isd:ied, jsd:jed, 1:nk));       cobalt%jn2o_sed_plus_btm=0.0
      allocate(cobalt%jsink_n2o_nit(isd:ied, jsd:jed, 1:nk));  cobalt%jsink_n2o_nit=0.0
      allocate(cobalt%jsink_n2o_denit(isd:ied, jsd:jed, 1:nk));cobalt%jsink_n2o_denit=0.0
      allocate(cobalt%jsink_n2o_atm(isd:ied, jsd:jed, 1:nk));  cobalt%jsink_n2o_atm=0.0
      allocate(cobalt%jsink_n2o_sed(isd:ied, jsd:jed, 1:nk));  cobalt%jsink_n2o_sed=0.0
      allocate(cobalt%tot_layer_int_n2o_nit(isd:ied, jsd:jed, 1:nk));   cobalt%tot_layer_int_n2o_nit=0.0
      allocate(cobalt%tot_layer_int_n2o_denit(isd:ied, jsd:jed, 1:nk)); cobalt%tot_layer_int_n2o_denit=0.0
      allocate(cobalt%tot_layer_int_n2o_atm(isd:ied, jsd:jed, 1:nk));   cobalt%tot_layer_int_n2o_atm=0.0
      allocate(cobalt%tot_layer_int_n2o_sed(isd:ied, jsd:jed, 1:nk));   cobalt%tot_layer_int_n2o_sed=0.0
      allocate(cobalt%b_n2o_nit(isd:ied, jsd:jed));            cobalt%b_n2o_nit=0.0
      allocate(cobalt%b_n2o_denit(isd:ied, jsd:jed));          cobalt%b_n2o_denit=0.0
      allocate(cobalt%b_n2o_atm(isd:ied, jsd:jed));            cobalt%b_n2o_atm=0.0
      allocate(cobalt%b_n2o_sed(isd:ied, jsd:jed));            cobalt%b_n2o_sed=0.0
      allocate(cobalt%btm_n2o_nit(isd:ied,jsd:jed));           cobalt%btm_n2o_nit = 0.0
      allocate(cobalt%btm_n2o_denit(isd:ied,jsd:jed));         cobalt%btm_n2o_denit = 0.0
      allocate(cobalt%btm_n2o_atm(isd:ied,jsd:jed));           cobalt%btm_n2o_atm = 0.0
      allocate(cobalt%btm_n2o_sed(isd:ied,jsd:jed));           cobalt%btm_n2o_sed = 0.0
      allocate(cobalt%wc_vert_int_n2o_nit(isd:ied, jsd:jed));  cobalt%wc_vert_int_n2o_nit=0.0
      ...
      allocate(cobalt%wc_vert_int_jsink_n2o_sed(isd:ied, jsd:jed));     cobalt%wc_vert_int_jsink_n2o_sed=0.0
      allocate(cobalt%stf_gas_n2o_nit(isd:ied, jsd:jed));      cobalt%stf_gas_n2o_nit=0.0
      allocate(cobalt%stf_gas_n2o_denit(isd:ied, jsd:jed));    cobalt%stf_gas_n2o_denit=0.0
      allocate(cobalt%stf_gas_n2o_atm(isd:ied, jsd:jed));      cobalt%stf_gas_n2o_atm=0.0
      allocate(cobalt%stf_gas_n2o_sed(isd:ied, jsd:jed));      cobalt%stf_gas_n2o_sed=0.0
      allocate(cobalt%deltap_n2o_nit(isd:ied, jsd:jed));       cobalt%deltap_n2o_nit=0.0
      allocate(cobalt%deltap_n2o_denit(isd:ied, jsd:jed));     cobalt%deltap_n2o_denit=0.0
      allocate(cobalt%deltap_n2o_atm(isd:ied, jsd:jed));       cobalt%deltap_n2o_atm=0.0
      allocate(cobalt%deltap_n2o_sed(isd:ied, jsd:jed));       cobalt%deltap_n2o_sed=0.0
      endif !}

      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      allocate(cobalt%f_n2o_diag(isd:ied, jsd:jed, 1:nk));              cobalt%f_n2o_diag=0.0
      allocate(cobalt%f_n2o_diag_nit(isd:ied, jsd:jed, 1:nk));          cobalt%f_n2o_diag_nit=0.0
      allocate(cobalt%f_n2o_diag_lowo2(isd:ied, jsd:jed, 1:nk));        cobalt%f_n2o_diag_lowo2=0.0
      allocate(cobalt%jn2o_diag(isd:ied, jsd:jed, 1:nk));               cobalt%jn2o_diag=0.0
      allocate(cobalt%jn2o_diag_nit(isd:ied, jsd:jed, 1:nk));           cobalt%jn2o_diag_nit=0.0
      allocate(cobalt%jn2o_diag_lowo2(isd:ied, jsd:jed, 1:nk));         cobalt%jn2o_diag_lowo2=0.0
      allocate(cobalt%jn2o_diag_plus_btm(isd:ied, jsd:jed, 1:nk));      cobalt%jn2o_diag_plus_btm=0.0
      allocate(cobalt%jn2o_diag_nit_plus_btm(isd:ied, jsd:jed, 1:nk));  cobalt%jn2o_diag_nit_plus_btm=0.0
      allocate(cobalt%jn2o_diag_lowo2_plus_btm(isd:ied, jsd:jed, 1:nk));cobalt%jn2o_diag_lowo2_plus_btm=0.0
      allocate(cobalt%jprod_n2o_diag(isd:ied, jsd:jed, 1:nk));          cobalt%jprod_n2o_diag=0.0
      allocate(cobalt%jprod_n2o_diag_nit(isd:ied, jsd:jed, 1:nk));      cobalt%jprod_n2o_diag_nit=0.0
      allocate(cobalt%jprod_n2o_diag_lowo2(isd:ied, jsd:jed, 1:nk));    cobalt%jprod_n2o_diag_lowo2=0.0
      allocate(cobalt%jsink_n2o_diag(isd:ied, jsd:jed, 1:nk));          cobalt%jsink_n2o_diag=0.0
      allocate(cobalt%jsink_n2o_diag_nit(isd:ied, jsd:jed, 1:nk));      cobalt%jsink_n2o_diag_nit=0.0
      allocate(cobalt%jsink_n2o_diag_lowo2(isd:ied, jsd:jed, 1:nk));    cobalt%jsink_n2o_diag_lowo2=0.0
      allocate(cobalt%tot_layer_int_n2o_diag(isd:ied, jsd:jed, 1:nk));  cobalt%tot_layer_int_n2o_diag=0.0
      allocate(cobalt%tot_layer_int_n2o_diag_nit(isd:ied, jsd:jed, 1:nk));  cobalt%tot_layer_int_n2o_diag_nit=0.0
      allocate(cobalt%tot_layer_int_n2o_diag_lowo2(isd:ied, jsd:jed, 1:nk));cobalt%tot_layer_int_n2o_diag_lowo2=0.0
      allocate(cobalt%b_n2o_diag(isd:ied, jsd:jed));                    cobalt%b_n2o_diag=0.0
      allocate(cobalt%b_n2o_diag_nit(isd:ied, jsd:jed));                cobalt%b_n2o_diag_nit=0.0
      allocate(cobalt%b_n2o_diag_lowo2(isd:ied, jsd:jed));              cobalt%b_n2o_diag_lowo2=0.0
      allocate(cobalt%n2o_diag_csurf(isd:ied, jsd:jed));                cobalt%n2o_diag_csurf=0.0
      allocate(cobalt%n2o_diag_nit_csurf(isd:ied, jsd:jed));            cobalt%n2o_diag_nit_csurf=0.0
      allocate(cobalt%n2o_diag_lowo2_csurf(isd:ied, jsd:jed));          cobalt%n2o_diag_lowo2_csurf=0.0
      allocate(cobalt%n2o_diag_alpha(isd:ied, jsd:jed));                cobalt%n2o_diag_alpha=0.0
      allocate(cobalt%n2o_diag_nit_alpha(isd:ied, jsd:jed));            cobalt%n2o_diag_nit_alpha=0.0
      allocate(cobalt%n2o_diag_lowo2_alpha(isd:ied, jsd:jed));          cobalt%n2o_diag_lowo2_alpha=0.0
      allocate(cobalt%wc_vert_int_n2o_diag(isd:ied, jsd:jed));          cobalt%wc_vert_int_n2o_diag=0.0
      allocate(cobalt%wc_vert_int_n2o_diag_nit(isd:ied, jsd:jed));      cobalt%wc_vert_int_n2o_diag_nit=0.0
      allocate(cobalt%wc_vert_int_n2o_diag_lowo2(isd:ied, jsd:jed));    cobalt%wc_vert_int_n2o_diag_lowo2=0.0
      allocate(cobalt%wc_vert_int_jn2o_diag(isd:ied, jsd:jed));         cobalt%wc_vert_int_jn2o_diag=0.0
      allocate(cobalt%wc_vert_int_jn2o_diag_nit(isd:ied, jsd:jed));     cobalt%wc_vert_int_jn2o_diag_nit=0.0
      allocate(cobalt%wc_vert_int_jn2o_diag_lowo2(isd:ied, jsd:jed));   cobalt%wc_vert_int_jn2o_diag_lowo2=0.0
      allocate(cobalt%wc_vert_int_jprod_n2o_diag(isd:ied, jsd:jed));    cobalt%wc_vert_int_jprod_n2o_diag=0.0
      allocate(cobalt%wc_vert_int_jprod_n2o_diag_nit(isd:ied, jsd:jed));cobalt%wc_vert_int_jprod_n2o_diag_nit=0.0
      allocate(cobalt%wc_vert_int_jprod_n2o_diag_lowo2(isd:ied, jsd:jed));cobalt%wc_vert_int_jprod_n2o_diag_lowo2=0.0
      allocate(cobalt%wc_vert_int_jsink_n2o_diag(isd:ied, jsd:jed));    cobalt%wc_vert_int_jsink_n2o_diag=0.0
      allocate(cobalt%wc_vert_int_jsink_n2o_diag_nit(isd:ied, jsd:jed));cobalt%wc_vert_int_jsink_n2o_diag_nit=0.0
      allocate(cobalt%wc_vert_int_jsink_n2o_diag_lowo2(isd:ied, jsd:jed));cobalt%wc_vert_int_jsink_n2o_diag_lowo2=0.0
      allocate(cobalt%stf_gas_n2o_diag(isd:ied, jsd:jed));              cobalt%stf_gas_n2o_diag=0.0
      allocate(cobalt%stf_gas_n2o_diag_nit(isd:ied, jsd:jed));          cobalt%stf_gas_n2o_diag_nit=0.0
      allocate(cobalt%stf_gas_n2o_diag_lowo2(isd:ied, jsd:jed));        cobalt%stf_gas_n2o_diag_lowo2=0.0
      allocate(cobalt%deltap_n2o_diag(isd:ied, jsd:jed));               cobalt%deltap_n2o_diag=0.0
      allocate(cobalt%deltap_n2o_diag_nit(isd:ied, jsd:jed));           cobalt%deltap_n2o_diag_nit=0.0
      allocate(cobalt%deltap_n2o_diag_lowo2(isd:ied, jsd:jed));         cobalt%deltap_n2o_diag_lowo2=0.0
      endif !} ! } YZ
   endif !} ! } YZ
  ...
  subroutine user_deallocate_arrays
    ...
    deallocate(bact(1)%no3lim)  ! YZ: 07/07/2025
    ...
    deallocate(cobalt%f_ndet_fast) ! YZ: fast-sinking, 07/07/2025
    deallocate(cobalt%f_nh4)
    deallocate(cobalt%f_no3)
    deallocate(cobalt%f_o2)
    deallocate(cobalt%f_pdet)
    deallocate(cobalt%f_pdet_fast) ! YZ: fast-sinking, 07/07/2025
    ...
    deallocate(cobalt%f_ndet_fast_btf) ! YZ: fast-sinking, 07/07/2025
    deallocate(cobalt%f_pdet_btf)
    deallocate(cobalt%f_pdet_fast_btf) ! YZ: fast-sinking, 07/07/2025
    ...
    deallocate(cobalt%jndet_fast) ! YZ: fast-sinking, 07/07/2025
    ...
    deallocate(cobalt%jpdet_fast) ! YZ: fast-sinking, 07/07/2025
    ...
    deallocate(cobalt%jprod_ndet_fast) ! YZ: fast-sinking, 07/07/2025
    deallocate(cobalt%jprod_pdet)
    deallocate(cobalt%jprod_pdet_fast) ! YZ: fast-sinking, 07/07/2025
...
!==============================================================================================================
    ...
    deallocate(cobalt%fpdet_fast_btm) ! YZ: fast-sinking, 07/07/2025
    deallocate(cobalt%fndet_btm)
    deallocate(cobalt%fndet_fast_btm) ! YZ: fast-sinking, 07/07/2025
    ...
    deallocate(cobalt%jremin_ndet_fast_100) ! YZ: fast-sinking, 07/07/2025
    deallocate(cobalt%f_ndet_100)
    deallocate(cobalt%f_ndet_fast_100) ! YZ: fast-sinking, 07/07/2025
    ...
    deallocate(cobalt%fndet_fast_100) ! YZ: fast-sinking, 07/07/2025
    deallocate(cobalt%fpdet_100)
    deallocate(cobalt%fpdet_fast_100) ! YZ: fast-sinking, 07/07/2025
...
!      deallocate(cobalt%runoff_flux_srdop)
   ...
   ! YZ: N2O_module, 13/06/2025 {
   if (do_n2o) then !{
      deallocate(cobalt%f_n2o)
      deallocate(cobalt%jn2o)
      deallocate(cobalt%jn2o_plus_btm)
      deallocate(cobalt%jprod_n2o_nit)
      deallocate(cobalt%jprod_n2o_denit)
      deallocate(cobalt%jsink_n2o)
      deallocate(cobalt%jremin_ndet_denit1)
      deallocate(cobalt%jremin_ndet_denit2)
      deallocate(cobalt%jremin_ndet_denit0)
      deallocate(cobalt%tot_layer_int_n2o)
      deallocate(cobalt%b_n2o)
      deallocate(cobalt%btm_n2o)
      deallocate(cobalt%n2osat)
      ...
      deallocate(cobalt%f_n2o_denit)
      deallocate(cobalt%f_n2o_atm)
      deallocate(cobalt%f_n2o_sed)
      deallocate(cobalt%jn2o_nit)
      deallocate(cobalt%jn2o_denit)
      deallocate(cobalt%jn2o_atm)
      deallocate(cobalt%jn2o_sed)
      deallocate(cobalt%jn2o_nit_plus_btm)
      deallocate(cobalt%jn2o_denit_plus_btm)
      deallocate(cobalt%jn2o_atm_plus_btm)
      deallocate(cobalt%jn2o_sed_plus_btm)
      deallocate(cobalt%jsink_n2o_nit)
      deallocate(cobalt%jsink_n2o_denit)
      deallocate(cobalt%jsink_n2o_atm)
      deallocate(cobalt%jsink_n2o_sed)
      deallocate(cobalt%tot_layer_int_n2o_nit)
      deallocate(cobalt%tot_layer_int_n2o_denit)
      deallocate(cobalt%tot_layer_int_n2o_atm)
      deallocate(cobalt%tot_layer_int_n2o_sed)
      deallocate(cobalt%b_n2o_nit)
      deallocate(cobalt%b_n2o_denit)
      deallocate(cobalt%b_n2o_atm)
      deallocate(cobalt%b_n2o_sed)
      deallocate(cobalt%btm_n2o_nit)
      deallocate(cobalt%btm_n2o_denit)
      deallocate(cobalt%btm_n2o_atm)
      deallocate(cobalt%btm_n2o_sed)
      deallocate(cobalt%wc_vert_int_n2o_nit)
      ...
      deallocate(cobalt%wc_vert_int_jsink_n2o_sed)
      deallocate(cobalt%stf_gas_n2o_nit)
      deallocate(cobalt%stf_gas_n2o_denit)
      deallocate(cobalt%stf_gas_n2o_atm)
      deallocate(cobalt%stf_gas_n2o_sed)
      deallocate(cobalt%deltap_n2o_nit)
      deallocate(cobalt%deltap_n2o_denit)
      deallocate(cobalt%deltap_n2o_atm)
      deallocate(cobalt%deltap_n2o_sed)
      endif !}

      ! YZ: N2O_diag, 06/12/2025 {
      if (do_n2o_diag) then !{
      deallocate(cobalt%f_n2o_diag)
      deallocate(cobalt%f_n2o_diag_nit)
      deallocate(cobalt%f_n2o_diag_lowo2)
      deallocate(cobalt%jn2o_diag)
      deallocate(cobalt%jn2o_diag_nit)
      deallocate(cobalt%jn2o_diag_lowo2)
      deallocate(cobalt%jn2o_diag_plus_btm)
      deallocate(cobalt%jn2o_diag_nit_plus_btm)
      deallocate(cobalt%jn2o_diag_lowo2_plus_btm)
      deallocate(cobalt%jprod_n2o_diag)
      deallocate(cobalt%jprod_n2o_diag_nit)
      deallocate(cobalt%jprod_n2o_diag_lowo2)
      deallocate(cobalt%jsink_n2o_diag)
      deallocate(cobalt%jsink_n2o_diag_nit)
      deallocate(cobalt%jsink_n2o_diag_lowo2)
      deallocate(cobalt%tot_layer_int_n2o_diag)
      deallocate(cobalt%tot_layer_int_n2o_diag_nit)
      deallocate(cobalt%tot_layer_int_n2o_diag_lowo2)
      deallocate(cobalt%b_n2o_diag)
      deallocate(cobalt%b_n2o_diag_nit)
      deallocate(cobalt%b_n2o_diag_lowo2)
      deallocate(cobalt%n2o_diag_alpha)
      deallocate(cobalt%n2o_diag_nit_alpha)
      deallocate(cobalt%n2o_diag_lowo2_alpha)
      deallocate(cobalt%n2o_diag_csurf)
      deallocate(cobalt%n2o_diag_nit_csurf)
      deallocate(cobalt%n2o_diag_lowo2_csurf)
      deallocate(cobalt%wc_vert_int_n2o_diag)
      deallocate(cobalt%wc_vert_int_n2o_diag_nit)
      deallocate(cobalt%wc_vert_int_n2o_diag_lowo2)
      deallocate(cobalt%wc_vert_int_jn2o_diag)
      deallocate(cobalt%wc_vert_int_jn2o_diag_nit)
      deallocate(cobalt%wc_vert_int_jn2o_diag_lowo2)
      deallocate(cobalt%wc_vert_int_jprod_n2o_diag)
      deallocate(cobalt%wc_vert_int_jprod_n2o_diag_nit)
      deallocate(cobalt%wc_vert_int_jprod_n2o_diag_lowo2)
      deallocate(cobalt%wc_vert_int_jsink_n2o_diag)
      deallocate(cobalt%wc_vert_int_jsink_n2o_diag_nit)
      deallocate(cobalt%wc_vert_int_jsink_n2o_diag_lowo2)
      deallocate(cobalt%stf_gas_n2o_diag)
      deallocate(cobalt%stf_gas_n2o_diag_nit)
      deallocate(cobalt%stf_gas_n2o_diag_lowo2)
      deallocate(cobalt%deltap_n2o_diag)
      deallocate(cobalt%deltap_n2o_diag_nit)
      deallocate(cobalt%deltap_n2o_diag_lowo2)
      endif !} ! } YZ
   endif !} ! } YZ
